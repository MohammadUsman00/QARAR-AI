...
- Added Supabase-backed durable fixed-window AI route rate limiting in `src/lib/rate-limit.ts`.
...
- Added crisis-input screening and output safety checks for high-risk generated advice in `src/lib/llm-safety.ts`.
...
- Added durable AI rate-limit table/RPC migration in `supabase/migrations/004_durable_ai_rate_limits.sql`.
- Added client-safe API error messages and baseline security headers.
...
- Run the new Supabase migrations (`003`, `004`) in the production database before deploying the updated AI routes.
...
- Add dashboard/alerting for telemetry fields: latency, token cost, parse failures, timeouts, rate-limit fallback mode, safety blocks, and provider errors.
